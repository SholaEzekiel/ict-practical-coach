...
import { ArrowRight, Code2, FileCode2, Image, Link2, MonitorCheck, Palette, Table2 } from "lucide-react";
import { Pill, Section } from "@/components/ui";
import { getWebsiteAuthoringCardsForModule, websiteAuthoringModules } from "@/lib/website-authoring-instruction-cards";
  ...
  return (
    <Section className="max-w-7xl">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {websiteAuthoringModules.map((module) => {
          const Icon = moduleIcons[module.id as keyof typeof moduleIcons] || Code2;
          const tasks = getWebsiteAuthoringCardsForModule(module.id);

          return (
            <Link key={module.id} href={`/subjects/ict/website-authoring/${module.id}`} className="rounded-lg border border-line bg-white p-5 shadow-sm transition hover:border-ocean hover:shadow-soft">
              ...
              <div className="mt-5 flex items-center justify-between text-sm">
                <span className="font-semibold text-slate-600">HTML/CSS checks</span>
